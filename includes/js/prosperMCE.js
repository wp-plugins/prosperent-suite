...
r=jQuery("#useTitle").is(":checked"),t=jQuery("#onSale").is(":checked"),u=jQuery("#gridimgsz").val(),v=jQuery("#prosperSC").val(),w=jQuery("#css").val(),x=jQuery("#searchFor:checked").val(),y=jQuery("#sBarText").val(),z=jQuery("#sButtonText").val(),A=jQuery("#pricerangea").val(),B=jQuery("#pricerangeb").val(),C=jQuery("#noShow:checked").val(),D=jQuery("#imageType").val(),E = jQuery("#width").val(),F = jQuery("#widthStyle:checked").val(),c=jQuery("#"+c+"category").val(),a="["+v;e&&(a+=' q="'+e+'"');k&&(a+=' gtm="'+k+'"');l&&(a+=' b="'+l+'"');f&&(a+=' m="'+f+'"');
g&&(a+=' l="'+g+'"');h&&(a+=' v="'+h+'"');n&&(a+=' id="'+n+'"');d&&(a+=' ft="'+d+'"');w&&(a+=' css="'+w+'"');p&&(a+=' q="'+p+'"');q&&(a+=' utg="'+q+'"');r&&(a+=' utt="'+r+'"');t&&(a+=' sale="'+t+'"');x&&(a+=' sf="'+x+'"');y&&(a+=' sbar="'+y+'"');z&&(a+=' sbu="'+z+'"');m&&(a+=' vst="'+m+'"');C&&(a+=' noShow="'+C+'"');D&&(a+=' imgt="'+D+'"');c&&(a+=' cat="'+c+'"');E && (a + ' w="' + E + '"');F && (a += ' ws="' + F + '"');(A||B)&&(a+=' pr="'+A+","+B+'"');u&&"prod"==b&&(a+=' gimgsz="'+u+'"');a+="]"+shortCode.local_ed.selection.getContent()+"[/"+v+"]";tinyMCEPopup.execCommand("mceReplaceContent",
...
/*
 * c = jQuery("#width").val(),
   C = jQuery("#widthStyle:checked").val(),
   a = c && "auto" != c ? a + (' w="' + c + '"') : a + ' w="auto"';
   C && (a += ' ws="' + C + '"');
 */
/*
var shortCode = {
