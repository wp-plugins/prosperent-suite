...
if(is_front_page())
{
	$prodSubmit = site_url('/') . 'products';
}

if (isset($_POST['q']))
{
	$newQuery = str_replace(array('/query/' . $query, '/query/' . rawurlencode($query), '/page/' . $pageNumber), array('', '', ''), $prodSubmit);
...
<div class="prosper_searchform">
    <form class="searchform" method="POST" action="">
        <input class="prosper_field" type="text" name="q" id="s" placeholder="<?php echo isset($options['Search_Bar_Text']) ? $options['Search_Bar_Text'] : 'Search Products'; ?>">
        ...
        foreach ($results as $i => $record)
        {
            ...
            <div class="<?php echo $i > 0 ? 'productBlock' : 'productBlock0'; ?>">
                <div class="productImage">
                    <a href="<?php echo $productPage . '/product/' . rawurlencode(str_replace('/', ',SL,', $record['keyword'])) . '/cid/' . $record['catalogId']; ?>"><span><img src="<?php echo $record['image_url']; ?>"  title="<?php echo $record['keyword']; ?>" alt="<?php echo $record['keyword']; ?>" style="background: none repeat scroll 0 0 transparent; border: medium none;"/></span></a>
                </div>
                <div class="productContent">
                    <div class="productTitle"><a href="<?php echo $productPage . '/product/' . rawurlencode(str_replace('/', ',SL,', $record['keyword'])) . '/cid/' . $record['catalogId']; ?>"><span><?php echo $record['keyword']; ?></span></a></div>
                    ...
                    <div class="productBrandMerchant">
                        ...
                        {
                            echo '<span class="brandIn"><u>Brand</u>: <a href="' . str_replace('/page/' . $pageNumber, '', $prodSubmit) . '/brand/' . rawurlencode($record['brand']) . '"><cite>' . $record['brand'] . '</cite></a></span>';
                        }
                        if($record['merchant'] && !$filterMerchant)
                        {
                            echo '<span class="merchantIn"><u>Merchant</u>: <a href="' . str_replace('/page/' . $pageNumber, '', $prodSubmit) . '/merchant/' . rawurlencode($record['merchant']) . '"><cite>' . $record['merchant'] . '</cite></a></span>';
                        }
                        ?>
                    </div>
                </div>
                <div class="productEnd">
                    ...
                    ?>
                    <form style="margin:0;" action="<?php echo $productPage . '/store/go/' . rawurlencode(str_replace('/', ',SL,', $record['affiliate_url'])) . '" target="' . $target; ?>" method="POST">
			...
			<tr>
				<td class="brands">
					...
					else if (!$filterBrand)
					{
						...
						{
							echo '<a href=' . str_replace('/page/' . $pageNumber, '', $prodSubmit) . '/brand/' . rawurlencode($brand['value']) . '>' . $brand['value'] . ' (' . $brand['count'] . ')</a>';
						...
						if ($brands2)
						{
							if ($filterMerchant)
							{
								...
								<a onclick="toggle_visibility('brandList'); toggle_hidden('moreBrands'); toggle_visibility('hideBrands'); return false;" style="cursor:pointer; font-size:12px;"><span id="moreBrands" style="display:block;">More Brands <img src="<?php echo PROSPER_URL . 'img/arrow_down_small.png'; ?>" alt="" style="background: none repeat scroll 0 0 transparent; border: medium none;"/></span></a>
								<a onclick="toggle_hidden('brandList'); toggle_hidden('hideBrands'); toggle_visibility('moreBrands'); return false;" style="cursor:pointer; font-size:12px;"><span id="hideBrands" style="display:none;">Hide Brands <img src="<?php echo PROSPER_URL . 'img/arrow_up_small.png'; ?>" alt="" style="background: none repeat scroll 0 0 transparent; border: medium none;"/></span></a>
								<?php
							}
							else
							{
					...
					else
					{
						...
						echo rawurldecode($filterBrand);
						echo '</br><a href=' . str_replace(array('/page/' . $pageNumber, '/brand/' . $filterBrand), array('', ''), $prodSubmit) . '>clear filter</a>';
						echo '<div style="margin-top:-50px;padding-left:150px;"><img src="' . ($options['Image_Masking'] ? $productPage  . '/img/' . rawurlencode(str_replace('/', ',SL,',  ('brandlogos/120x60/' . $filterBrand . '.png'))) : 'http://img1.prosperent.com/images/brandlogos/120x60/' . $filterBrand . '.png') . '" alt="' . $filterBrand . '" style="background: none repeat scroll 0 0 transparent; border: medium none;"/></div>';
						echo '</div>';
					}
					?>
				</td>
				<td class="merchants">
					...
					else if (!$filterMerchant)
					{
						...
						{
							echo '<a href=' . str_replace('/page/' . $pageNumber, '', $prodSubmit) . '/merchant/' . rawurlencode($merchant['value']) . '>' . $merchant['value'] . ' (' . $merchant['count'] . ')</a>';
						...
						if ($merchants2)
						{
							if ($filterBrand)
							{
								...
								<a onclick="toggle_visibility('merchantList'); toggle_hidden('moreMerchants'); toggle_visibility('hideMerchants'); return false;" style="cursor:pointer; font-size:12px;"><span id="moreMerchants" style="display:block;">More Merchants <img src="<?php echo PROSPER_URL . 'img/arrow_down_small.png'; ?>" alt="" style="background: none repeat scroll 0 0 transparent; border: medium none;"/></span></a>
								<a onclick="toggle_hidden('merchantList'); toggle_hidden('hideMerchants'); toggle_visibility('moreMerchants'); " style="cursor:pointer; font-size:12px;"><span id="hideMerchants" style="display:none;">Hide Merchants <img src="<?php echo PROSPER_URL . 'img/arrow_up_small.png'; ?>" alt="" style="background: none repeat scroll 0 0 transparent; border: medium none;"/></span></a>
								<?php
							}
							else
							{
					...
					else
					{
						...
						echo '<div style="margin-top:-50px;padding-left:150px;"><img src="' . ($options['Image_Masking'] ? $productPage  . '/img/' . rawurlencode(str_replace('/', ',SL,',  ('logos/120x60/' . $filterMerchant . '.png'))) : 'http://img1.prosperent.com/images/logos/120x60/' . $filterMerchant . '.png') . '" alt="' . $filterMerchant . '" style="background: none repeat scroll 0 0 transparent; border: medium none;"/></div>';
						echo '</div>';
					}
					?>
				</td>
		...
		if ($brands2)
		{
			...
			<table id="brandList" style="display:none; font-size:11px; width:100%; table-layout:fixed;">
				...
				foreach ($brands2 as  $i => $brand)
				{
					...
					echo '<td style="width:1%; padding:5px; height:30px;"><a href=' . str_replace('/page/' . $pageNumber, '', $prodSubmit) . '/brand/' . rawurlencode($brand['value']) . '>' . $brand['value'] . ' (' . $brand['count'] . ')</a></td>';
		...
		if ($merchants2)
		{
			...
			<table id="merchantList" style="display:none; font-size:11px; width:100%;">
				...
				foreach ($merchants2 as $i => $merchant)
				{
					...
					echo '<td style="padding:5px; height:30px; width:1%;"><a href=' . str_replace('/page/' . $pageNumber, '', $prodSubmit) . '/merchant/' . rawurlencode($merchant['value']) . '>' . $merchant['value'] . ' (' . $merchant['count'] . ')</a></td>';
