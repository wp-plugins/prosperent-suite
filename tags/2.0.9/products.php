...
foreach ($endpoints as $endpoint)
{
	...
	switch ($endpoint)
	{
		...
		case 'Celebrity_Endpoint':
			$sepEnds['cele'] = '<a href="' . str_replace(array('/city/' . $filterCity, '/state/' . $filterState, '/zip/' . $filterZip, '/page/' . $pageNumber, '/type/' . $type, '/query/' . $query, '/sort/' . $sort), array('', '', '', '', '', '', '',), $url) . '/type/cele">' . $celebrityTitle . '</a>';
			break;
		case 'Local_Endpoint':
