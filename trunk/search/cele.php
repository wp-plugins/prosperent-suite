...
$newQuery = str_replace(array('/query/' . $query, '/celebQuery/' . $celebQuery), array('', ''), $celeSubmit);
$newSort = str_replace('/sort/' . $sendParams['sort'], '', $celeSubmit);
    ...
    <tr>
        <td class="merchants" style="width:98%; float:none;">
            ...
            else
            {
                ...
                echo $celebDecode;
                echo '</br><a href=' . str_replace(array('/merchant/' . $filterMerchant, '/brand/' . $filterBrand, '/celeb/' . $celeb), array('', '', ''), $celeSubmit) . ' >clear filter</a>';
...
if (empty($results))
{
    ...
    {
        echo '<div class="noResults-secondary">Please try your search again or <a style="text-decoration:none;" href=' . str_replace(array('/merchant/' . $filterMerchant, '/brand/' . $filterBrand), array('', ''), $celeSubmit) . '>clear the filter(s)</a>.</div>';
    }
    else
    {
...
else
{
    ...
    echo '<div class="totalFound">' . $totalFound . ' results for <b>' . ucwords($celebDecode) . ($celebQuery ? ' + ' . $celebQuery : '') . '</b></div>';
    ?>
