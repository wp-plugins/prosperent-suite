<?php
if (!$query && $options['Starting_Query'])
{
		...
		<tr>
			<td class="brands">
				...
				else
				{
					...
					echo '</br><a href=' . str_replace('/brand/' . $filterBrand, '', $prodSubmit) . '>clear filter</a>';
					echo '<div style="margin-top:-50px;padding-left:150px;"><img src="' . $productPage  . '/img/' . urlencode(str_replace('/', ',SL,',  ('brandlogos/120x60/' . $filterBrand . '.png'))) . '" style="background: none repeat scroll 0 0 transparent; border: medium none;"/></div>';
					echo '</div>';					
				}
				?>
			</td>
			<td class="merchants">
				...
				else if (!$filterMerchant)
				{
					...
					{
						echo '<a href=' . $prodSubmit . '/merchant/' . urlencode($merchant['value']) . '>' . $merchant['value'] . ' (' . $merchant['count'] . ')</a>';
				...
				else
				{
					...
					echo '</br><a href=' . str_replace('/merchant/' . $filterMerchant, '', $prodSubmit) . '>clear filter</a>';
	...
	if ($brands2)
	{
		...
		<table id="brandList" style="display:none; font-size:11px; width:100%; table-layout:fixed;">
			...
			foreach ($brands2 as  $i => $brand)
			{
				...
				echo '<td style="width:1%; padding:5px; height:30px;"><a href=' . $prodSubmit . '/brand/' . urlencode($brand['value']) . '>' . $brand['value'] . ' (' . $brand['count'] . ')</a></td>';
	...
	if ($merchants2)
	{
		...
		<table id="merchantList" style="display:none; font-size:11px; width:100%;">
			...
			foreach ($merchants2 as $i => $merchant)
			{
				...
				echo '<td style="padding:5px; height:30px; width:1%;"><a href=' . $prodSubmit . '/merchant/' . urlencode($merchant['value']) . '>' . $merchant['value'] . ' (' . $merchant['count'] . ')</a></td>';
